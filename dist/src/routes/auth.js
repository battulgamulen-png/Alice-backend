...
router.get("/auth/signup/check", async (req, res) => {
  ...
  if (!email && !cardNumber) {
    return (0, http_1.sendJson)(res, 400, {
      error: "Provide email or cardNumber",
    });
  }
  ...
  try {
    ...
    console.error(err);
    return (0, http_1.sendJson)(res, 500, {
      error: "Could not validate signup fields",
    });
  }
});
    ...
    if (err instanceof client_1.Prisma.PrismaClientKnownRequestError) {
      if (err.code === "P2002") {
        ...
        if (target.includes("email")) {
          return (0, http_1.sendJson)(res, 409, {
...
router.post("/auth/forgot-password", async (req, res) => {
  ...
  if (!user || !user.phone || user.phone.trim() !== phone.trim()) {
    return (0, http_1.sendJson)(res, 401, {
      error: "Email or phone does not match",
    });
  }
